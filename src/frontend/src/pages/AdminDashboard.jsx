...
export default function AdminDashboard() {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto', padding: '24px' }}>
      <h2 style={{ color: '#fff', fontSize: '20px', fontWeight: 700, marginBottom: '24px' }}>⚙️ {t('admin.title')}</h2>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px', marginBottom: '28px' }}>
        <StatCard icon="🆘" value={stats.pendingRequests} label={t('admin.pendingRequests')} color="#f87171" />
        <StatCard icon="✅" value={stats.resolvedRequests} label={t('admin.resolvedToday')} color="#4ade80" />
        <StatCard icon="🙋" value={stats.totalVolunteers} label={t('admin.volunteersActive')} color="#60a5fa" />
        <StatCard icon="🏢" value={stats.activeNGOs} label={t('admin.verifiedNgos')} color="#fbbf24" />
        <StatCard icon="📋" value={stats.totalRequests} label={t('admin.totalRequests')} color="#a78bfa" />
        <StatCard icon="⏳" value={ngos.length} label={t('admin.ngosPending')} color="#fb923c" />
      </div>

      {/* NGO Verification */}
      <div style={{ background: '#111827', border: '1px solid #1e3a5f', borderRadius: '14px', padding: '20px', marginBottom: '20px' }}>
        <h3 style={{ color: '#fbbf24', fontSize: '14px', fontWeight: 700, marginBottom: '14px' }}>🏢 {t('admin.ngoVerificationQueue')} ({ngos.length})</h3>
        {ngos.length === 0 && <p style={{ color: '#4b5563', fontSize: '13px' }}>{t('admin.noPendingNgos')}</p>}
        {ngos.map(n => (
          <div key={n._id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 0', borderBottom: '1px solid #1f2937' }}>
            ...
            <div style={{ display: 'flex', gap: '8px' }}>
              <button onClick={() => handleVerify(n._id, true)} style={{ background: '#14532d', color: '#4ade80', border: '1px solid #22c55e', borderRadius: '8px', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontWeight: 700 }}>✅ {t('admin.approve')}</button>
              <button onClick={() => handleVerify(n._id, false)} style={{ background: '#7f1d1d', color: '#f87171', border: '1px solid #ef4444', borderRadius: '8px', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontWeight: 700 }}>✕ {t('admin.reject')}</button>
            </div>
          </div>
        ))}
      </div>
      ...
      <div style={{ background: '#111827', border: '1px solid #1e3a5f', borderRadius: '14px', padding: '20px' }}>
        <h3 style={{ color: '#f87171', fontSize: '14px', fontWeight: 700, marginBottom: '14px' }}>📣 {t('admin.emergencyBroadcast')}</h3>
        <input value={broadcast.title} onChange={e => setBroadcast(b => ({ ...b, title: e.target.value }))} placeholder={t('admin.broadcastTitle')}
        ...
        <textarea value={broadcast.message} onChange={e => setBroadcast(b => ({ ...b, message: e.target.value }))} placeholder={t('admin.broadcastMessage')} rows={3}
