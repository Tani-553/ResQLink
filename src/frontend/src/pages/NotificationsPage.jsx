    ...
    <div style={{ maxWidth: '700px', margin: '0 auto', padding: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2 style={{ color: '#fff', fontSize: '20px', fontWeight: 700, margin: 0 }}>
          🔔 {t('navbar.notifications')} {unreadCount > 0 && <span style={{ background: '#ef4444', color: '#fff', borderRadius: '20px', padding: '2px 10px', fontSize: '12px', marginLeft: '8px' }}>{unreadCount}</span>}
        </h2>
        {unreadCount > 0 && (
      ...
      {loading && <p style={{ color: '#6b7280' }}>{t('notifications.loading')}</p>}
      {!loading && notifications.length === 0 && <p style={{ color: '#4b5563', fontSize: '14px' }}>{t('notifications.noNotifications')}</p>}
