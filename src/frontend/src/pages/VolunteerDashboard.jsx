...
export default function VolunteerDashboard() {
  ...
  const fetchNearby = () => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      ...
      setLoading(false);
    }, () => { setLocError(t('volunteer.enableGPS')); setLoading(false); });
  };
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '24px' }}>
      <h2 style={{ color: '#fff', fontSize: '20px', fontWeight: 700, marginBottom: '20px' }}>🙋 {t('volunteer.title')}</h2>
      ...
      {tab === 'nearby' && (
        <>
          ...
          {requests.length === 0 && !loading && <p style={{ color: '#4b5563', fontSize: '14px' }}>{t('volunteer.noNearby')}</p>}
          {requests.map(r => (
            <div key={r._id} style={{ background: '#111827', border: `1px solid ${priorityColor[r.priority]}44`, borderLeft: `4px solid ${priorityColor[r.priority]}`, borderRadius: '12px', padding: '16px', marginBottom: '12px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ color: '#60a5fa', fontSize: '12px' }}>📍 {r.location?.address || t('volunteer.attached')}</span>
