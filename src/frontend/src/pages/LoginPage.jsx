...
const ROLES = [
  { value: 'victim',    labelKey: 'login.victim',    color: '#E53E3E' },
  { value: 'volunteer', labelKey: 'login.volunteer',  color: '#3B82F6' },
  { value: 'ngo',       labelKey: 'login.ngo',        color: '#38A169' },
  { value: 'admin',     labelKey: 'login.admin',      color: '#D69E2E' },
];
    ...
    <div style={{ minHeight: '100vh', background: '#0a0f1e', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
      <div style={{ background: '#111827', border: '1px solid #1e3a5f', borderRadius: '16px', padding: '40px', width: '100%', maxWidth: '400px' }}>
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <h1 style={{ color: '#fff', fontSize: '20px', fontWeight: 700, margin: 0 }}>{t('login.title')}</h1>
          <p style={{ color: '#6b7280', fontSize: '13px', margin: '4px 0 0' }}>
            {isRegister ? t('login.registerPrompt') : t('login.loginPrompt')}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          {isRegister && (
            <>
              <input name="name" value={form.name} onChange={handleChange} placeholder={t('login.fullName')} required
                style={inputStyle} />
              <input name="phone" value={form.phone} onChange={handleChange} placeholder={t('login.phone')} required
                style={inputStyle} />
            </>
          )}
          <input name="email" type="email" value={form.email} onChange={handleChange} placeholder={t('login.email')} required style={inputStyle} />
          <input name="password" type="password" value={form.password} onChange={handleChange} placeholder={t('login.password')} required style={inputStyle} />

          {isRegister && (
            <div style={{ marginBottom: '16px' }}>
              <p style={{ color: '#9ca3af', fontSize: '12px', marginBottom: '8px' }}>{t('login.selectRole')}</p>
          ...
          <button type="submit" disabled={loading}
            ...
            {loading ? t('login.loading') : isRegister ? t('login.createAccount') : t('login.loginButton')}
          </button>
        </form>
