...
export default function NGODashboard() {
  ...
  if (!profile) return (
    <div style={{ maxWidth: '700px', margin: '40px auto', padding: '24px', background: '#111827', border: '1px solid #1e3a5f', borderRadius: '14px' }}>
      <h3 style={{ color: '#fbbf24', fontWeight: 700, marginBottom: '12px' }}>🏢 Complete Your NGO Profile</h3>
      <p style={{ color: '#9ca3af', fontSize: '13px' }}>Your NGO registration is pending admin verification. Please ensure your documents are uploaded.</p>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '24px' }}>
      ...
      <h3 style={{ color: '#9ca3af', fontSize: '12px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '14px' }}>Resource Inventory</h3>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '12px', marginBottom: '16px' }}>
        <ResourceBox label="Relief Kits" key="reliefKits" icon="🎒" color="#60a5fa" />
        <ResourceBox label="Shelters" key="shelters" icon="🏕️" color="#4ade80" />
        <ResourceBox label="Vehicles" key="vehicles" icon="🚐" color="#fbbf24" />
        <ResourceBox label="Medical" key="medicalSupplies" icon="💊" color="#f87171" />
      </div>
      ...
      <h3 style={{ color: '#9ca3af', fontSize: '12px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', marginBottom: '12px' }}>Active Zones ({profile.activeZones?.length || 0})</h3>
      <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
        ...
        {!profile.activeZones?.length && <p style={{ color: '#4b5563', fontSize: '13px' }}>No active zones assigned yet.</p>}
      </div>
    </div>
  );
}
