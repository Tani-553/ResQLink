...
export default function AdminDashboard() {
  ...
  const StatCard = ({ value, label, color }) => (
    <div style={{ background: '#111827', border: '1px solid #1e3a5f', borderRadius: '12px', padding: '20px', textAlign: 'center' }}>
      ...
      <div style={{ color: '#6b7280', fontSize: '12px', marginTop: '6px' }}>{label}</div>
    </div>
  );
          ...
          <div key={ngo._id} style={{ padding: '16px 0', borderBottom: '1px solid #1f2937' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '18px', marginBottom: '12px' }}>
              <div>
                ...
                <div style={{ color: '#6b7280', fontSize: '12px', marginTop: '6px', maxWidth: '520px', lineHeight: 1.5 }}>
                  {ngo.description || 'No NGO description submitted.'}
                </div>
              </div>
              <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                <button onClick={() => handleVerify(ngo._id, true)} style={{ background: '#14532d', color: '#4ade80', border: '1px solid #22c55e', borderRadius: '8px', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontWeight: 700 }}>
                  Approve
                </button>
                <button onClick={() => handleVerify(ngo._id, false)} style={{ background: '#7f1d1d', color: '#f87171', border: '1px solid #ef4444', borderRadius: '8px', padding: '6px 14px', fontSize: '12px', cursor: 'pointer', fontWeight: 700 }}>
                  Reject
                </button>
              </div>
            </div>
